...
const ClientCard: React.FC<{
  ...
  const [, drop] = useDrop({
    accept: ITEM,
    hover: (item: { index: number }) => { if (item.index !== index) { onMove(item.index, index); item.index = index; } },
  });
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => navigate(`/admin/client/${client.id}`)}
        ...
        >
          {client.status === 'archived' ? <RotateCcw className="w-4 h-4 text-green-400" /> : <Archive className="w-4 h-4" />}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <div className="animate-fade-in">
        <PageHeader title="Client Dashboard" description="Manage all contractor accounts"
        ...
        {loading ? (
          ...
          <EmptyState icon={<Users className="w-12 h-12" />} title="No clients found"
        ...
        <Modal open={addModal} onClose={() => setAddModal(false)} title="Add New Client" size="md">
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <Input label="Business Name *" value={addForm.businessName} onChange={(e) => setAddForm({ ...addForm, businessName: e.target.value })} placeholder="ABC Logistics LLC" />
              <Input label="Email *" type="email" value={addForm.email} onChange={(e) => setAddForm({ ...addForm, email: e.target.value })} placeholder="contact@company.com" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <Input label="Contact Name" value={addForm.contactName} onChange={(e) => setAddForm({ ...addForm, contactName: e.target.value })} placeholder="John Smith" />
              <Input label="Phone" value={addForm.phone} onChange={(e) => setAddForm({ ...addForm, phone: e.target.value })} placeholder="(555) 000-0000" />
            </div>
            <Select label="Contractor Type" value={addForm.contractorType}
              onChange={(e) => setAddForm({ ...addForm, contractorType: e.target.value })}
              options={[{ value: 'P&D', label: 'P&D' }, { value: 'Linehaul', label: 'Linehaul' }, { value: 'Both', label: 'Both' }]} />
